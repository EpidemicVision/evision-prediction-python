...
import pandas as pd
from evision.influenza.constants import TRENDS_NATIONAL_CSV, TRENDS_STATE_CSV, ILINET_NATIONAL_CSV, ILINET_STATE_CSV
...
def fetch_data(terms: List, level: str, states: str = None) -> pd.DataFrame:
    ...
    if level == "National":
        ...
        trends_df = trends_df.loc[trends_df['state']==states, :].drop(columns = ["state","state_code"])
        ilinet_df = pd.read_csv(ILINET_STATE_CSV, skiprows=[0], na_values='X')
        ilinet_df = ilinet_df.loc[ilinet_df['REGION']==states, :]

    ilinet_df.drop(ilinet_df.loc[ilinet_df['ILITOTAL'].isnull()].index, axis=0, inplace=True)
    terms.extend(["date", "week_number", "year"])
    trends_df = trends_df[terms]
    df = pd.merge(trends_df, ilinet_df.loc[:, ["YEAR", "WEEK","ILITOTAL"]], left_on=["year", "week_number"], right_on=["YEAR", "WEEK"], how="inner").drop(columns=["week_number", "year", "YEAR", "WEEK"])
    logging.info(f'Combined data: {df.shape}')
    return df


# @st.cache_data
def influenza_train_and_predict(data: pd.DataFrame, epochs: int, predict_ahead_by: int) -> Dict[str, Any]:
    dates = data['date'].to_list()
    data.drop(columns=["date"], inplace=True)
    data = data.astype(float)
    pred_col = 'ILITOTAL'
    batch_size = 256
    X, y = data.iloc[:-predict_ahead_by, :].copy(deep=True), data.iloc[predict_ahead_by:,:].loc[:, [pred_col]].copy(deep=True)
    ...
    history = model1.fit(trainX, trainy, batch_size=batch_size, epochs=epochs, shuffle=False)
    ...
    ci = smape(pred_unnorm.reshape(testy.shape[0]),testy_unnorm.reshape(testy.shape[0]))
