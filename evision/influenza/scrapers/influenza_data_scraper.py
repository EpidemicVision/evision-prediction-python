...
from evision.influenza.constants import INFLUENZA_DATA_DIR, TRENDS_KEYWORDS, STATE_CODE_MAPPER
...
year_2022_id = 62 # the ID for year 2022 is 62 on CDC. We need ID to fetch data based on year
...
def cdc_ilinet_downloader(download_dir: str = INFLUENZA_DATA_DIR):
    ...
    """
    print(f"Beginning downloading CDC ILINet data for National Level. Following are the start and end SeasonsDT params {start_id}-{end_id}")

    request_dict_states = {
        "AppVersion":"Public","DatasourceDT":[{"ID":0,"Name":"WHO_NREVSS"},{"ID":1,"Name":"ILINet"}],"RegionTypeId":5,
        "SubRegionsDT":[{"ID":1,"Name":"1"},{"ID":2,"Name":"2"},{"ID":3,"Name":"3"},{"ID":4,"Name":"4"},{"ID":5,"Name":"5"},{"ID":6,"Name":"6"},{"ID":7,"Name":"7"},{"ID":8,"Name":"8"},{"ID":9,"Name":"9"},{"ID":10,"Name":"10"},{"ID":11,"Name":"11"},{"ID":12,"Name":"12"},{"ID":13,"Name":"13"},{"ID":14,"Name":"14"},{"ID":15,"Name":"15"},{"ID":16,"Name":"16"},{"ID":17,"Name":"17"},{"ID":18,"Name":"18"},{"ID":19,"Name":"19"},{"ID":20,"Name":"20"},{"ID":21,"Name":"21"},{"ID":22,"Name":"22"},{"ID":23,"Name":"23"},{"ID":24,"Name":"24"},{"ID":25,"Name":"25"},{"ID":26,"Name":"26"},{"ID":27,"Name":"27"},{"ID":28,"Name":"28"},{"ID":29,"Name":"29"},{"ID":30,"Name":"30"},{"ID":31,"Name":"31"},{"ID":32,"Name":"32"},{"ID":33,"Name":"33"},{"ID":34,"Name":"34"},{"ID":35,"Name":"35"},{"ID":36,"Name":"36"},{"ID":37,"Name":"37"},{"ID":38,"Name":"38"},{"ID":39,"Name":"39"},{"ID":40,"Name":"40"},{"ID":41,"Name":"41"},{"ID":42,"Name":"42"},{"ID":43,"Name":"43"},{"ID":44,"Name":"44"},{"ID":45,"Name":"45"},{"ID":46,"Name":"46"},{"ID":47,"Name":"47"},{"ID":48,"Name":"48"},{"ID":49,"Name":"49"},{"ID":50,"Name":"50"},{"ID":51,"Name":"51"},{"ID":52,"Name":"52"},{"ID":54,"Name":"54"},{"ID":55,"Name":"55"},{"ID":56,"Name":"56"},{"ID":58,"Name":"58"},{"ID":59,"Name":"59"}],
        "SeasonsDT":[{"ID":i,"Name":str(i)} for i in range(start_id, end_id)]
    }

    request_dict_national = {"AppVersion":"Public","DatasourceDT":[{"ID":0,"Name":"WHO_NREVSS"},{"ID":1,"Name":"ILINet"}],"RegionTypeId":3,"SeasonsDT":[{"ID":i,"Name":str(i)} for i in range(start_id, end_id)]}
    ...
    print(f"[INFO]: Finished downloading ILINET National data. Extracting it in {download_file_path}")
    # writing zip
    ...
    # renaming the CSV
    os.rename(os.path.join(download_dir, 'ILINet.csv'), os.path.join(download_dir, 'ILINet-National.csv'))
    ...
    print(f"[INFO]: Finished downloading ILINET State data. Extracting it in {download_file_path}")
    # writing zip
    with open(download_file_path, 'wb') as f:
        f.write(resp.content)
    ...
    # renaming the CSV
    os.rename(os.path.join(download_dir, 'ILINet.csv'), os.path.join(download_dir, 'ILINet-State.csv'))
    os.remove(download_file_path)
...
def trends_scraper(download_dir: str = INFLUENZA_DATA_DIR) -> None:
    ...
    for state_name, state_code in STATE_CODE_MAPPER.items():
        print(f"[INFO] Beginning trends scraping with {terms}, {state_code}, {timeframe}")
    ...
    complete_df.to_csv(os.path.join(download_dir, 'google_trends-State.csv'), index=False)
